<template>
  <NuxtLink to="/" class="p-4 transition-200 rounded-lg group odd:bg-blue-600 even:bg-white-700 odd:hover:bg-blue-600 even:hover:bg-white-800 w-full h-full flex flex-col justify-between">
    <div>
    ...
    <div class="transition-200 border-t border-t-gray-300 group-odd:border-t-white/20 pt-4 flex-y-center mt-auto">
      ...
      <div class="ml-2">
        ...
        <div class="flex-y-center mt-1">
          <p class="group-odd:text-white/60 group-even:text-gray flex-y-center text-sm leading-130">
            <i class="icon-eye group-odd:text-white/60 group-even:text-gray mr-1"></i> {{ formatNumberWithSpaces(data.views_count) }}
          </p>
          <span class="w-1 h-1 rounded-full bg-gray mx-1.5"></span>
          <p class="group-odd:text-white/60 group-even:text-gray flex-y-center text-sm leading-130"> {{ getTimeText(data.created_at, $t) }} </p>
        </div>
      </div>
    </div>
  </NuxtLink>
