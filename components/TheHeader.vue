<template>
    <div class="header__top">
        ...
        <nav class="header__nav">
            <ul>
                <li class="header__nav-item">
                    <nuxt-link :to="{ path: localePath('/') }">{{ $t('home') }}</nuxt-link>
                </li>
                <li class="header__nav-item">
                    <nuxt-link :to="{ path: localePath('/solutions') }">{{ $t('solutions') }}</nuxt-link>
                </li>
                <li class="header__nav-item">
                    <nuxt-link :to="{ path: localePath('/company') }">{{ $t('company') }}</nuxt-link>
                </li>
                <li class="header__nav-item">
                    <nuxt-link :to="{ path: localePath('/contacts')}">{{ $t('contacts') }}</nuxt-link>
                </li>
            </ul>
        </nav>
        <div class="header__buttons">
            ...
            <div v-if="langVisibility || currentLang !== 'us'" @click="isLang = !isLang" class="header__lang">
                <span>{{ activeLang?.value ? activeLang?.value : 'English' }}</span>
                <svg class="MuiSvgIcon-root" focusable="false" viewBox="0 0 24 24" aria-hidden="true" role="presentation"><path d="M7.41 8.59L12 13.17l4.59-4.58L18 10l-6 6-6-6 1.41-1.41z" fill="#fff"></path></svg>
                <ul v-if="isLang" class="header__lang-list">
                    <div >
                        <li v-for="(item, index) in langList" :key="index" @click="changeLanguage(item)">
                            <span>{{ item.value }}</span>
                        </li>
                    </div>
                </ul>
            </div>
            <button v-if="!route.path.startsWith('/us') || currentLang !== 'us'" @click="openModalFn" class="region-button">
                <span>{{ activeRegionTitle ? activeRegionTitle : 'Choose Region' }}</span>
            ...
            <a v-if="route.path != '/'" class="mobile-hide" href="mailto:hello@parqour.com">
            ...
            <div class="burger-menu">
                <ul class="burger-menu__ul">
                    <li @click="toggleMenu"
                        ...
                        <nuxt-link :to="{ path: localePath('/contacts'),  }">{{ $t('home') }}</nuxt-link>
                    </li>
                    <li @click="toggleMenu"
                        class="burger-menu__ul-li">
                        <nuxt-link :to="{ path: localePath('/solutions') }">{{ $t('solutions') }}</nuxt-link>
    ...
    <div v-if="modalVisibility" class="region">
        <div class="region-modal">
            ...
            >
                <div>
                    <i>
                        <img :src="item.icon" alt="icon" :class="activeRegion == index ? 'hovered' : ''">
                    </i>
                    <h3>{{ item.title }}</h3>
                </div>
                <svg v-if="activeRegion == index" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                    <path d="M10.0007 15.1709L19.1931 5.97852L20.6073 7.39273L10.0007 17.9993L3.63672 11.6354L5.05093 10.2212L10.0007 15.1709Z" fill="#157FFF"/>
                </svg>
            </div>
        </div>
        <div @click="modalVisibility = false" class="region-bg"></div>
    </div>
</template>
...
let isLang = ref(false);
let langList = ref([{ key: "en", value: "English" }])
let modalData = ref([
    {
        title: "Central Asia",
        active: false,
            ...
            navigator.geolocation.getCurrentPosition(position => {
                ...
                fetch(`https://api.opencagedata.com/geocode/v1/json?q=${latitude}+${longitude}&key=${apiKey}`)
                    ...
                    .then(data => {
                        ...
                        if (countryCode === 'US') {                            
                            setLocaleCookie('us')
                            router.push(`${route.fullPath.startsWith('/us') ? route.fullPath : `/us` + route.fullPath}`)
